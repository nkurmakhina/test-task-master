...
public class ExternalApproveService {
    ...
    @Autowired
    CostAssessmentService costService;
    ...
    public int approve(CollateralObject object) {
        ...
        switch (object.getType()) {
            ...
            case AIRPLANE:
                ...
                break;
            case COST_ASSESSMENT:
                code = approveCost(object);
                break;
            default:
                code = -100;
        }
    ...
    private int approveCost (CollateralObject object) {
        if(object.getDate().isAfter(LocalDate.now())){
            return -23;
        }
        LocalDate date = getCurrentlyModelDate(object);
        if (date != null && date.isAfter(object.getDate())){
            return -24;
        }
        return 0;
    }

    private LocalDate getCurrentlyModelDate(CollateralObject object){
        CostAssessment costFromModel = costService.load(object.getId()).get();
        return costFromModel.getLocalDate();
    }
}
